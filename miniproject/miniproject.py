...
def bubblesort():
    ...
    # ปุ่มคำนวณ
    button1 = Button(bubblesort, text="คำนวณ", font=3,bg="blue",fg="white", command=merge_cal)
...
def Mergesort():
    ...
    def merge_sort(arr, reverse=False): #ฟังชั่น merge_sort รับค่า arr และค่า reverse ,arr = รับมาจาก def merge_cal() ,โดยใน def จะเรียกใช้ merge_sortโดยส่งค่าinput_list และ reverse ไป 
        if len(arr) > 1: #ให้เช็คโดยนับค่าของ arr ว่ามีกี่ตัว โดย len จะทำการนับ จำนวน index เริ่มจาก 1 ,ซึ่งถ้าตัวที่เช็ค > 1 เข้าเงื่อนไขแรก
            ...
            while i < len(left_half) and j < len(right_half): #เป็นการสร้างลูป while โดยถ้า i > indexของleft_half และ j < index ของ right_half
                if (left_half[i] > right_half[j]) if reverse else (left_half[i] < right_half[j]): # เปรียบเทียบค่า left_half[i] กับ right_half[j] ซึ่งขึ้นอยู่กับค่าของ reverse 
                    # ***กรณีที่ (left_half[i] > right_half[j])
                    arr[k] = left_half[i] #ให้ left_half ที่ index i ไปแทนใน ตำแหน่งที่ k ของ arr 
                    i += 1 #ให้ i = i + 1
                else: # ***กรณีที่ (left_half[i] < right_half[j])
