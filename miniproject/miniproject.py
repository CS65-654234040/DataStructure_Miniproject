...
def Insertionsort():
    ...
    def insertionsortcal():
        ...
        insertion1 = Label(text="โปรแกรมการแทรกเรียงลำดับ", font=20, fg="white", bg="Blue")
        insertion1.grid(row=0, column=2, columnspan=5, padx=150, pady=20)

        label1 = Label(text="กรอกข้อมูล", font=20, fg="white", bg="black")
        label1.grid(row=1, column=3, columnspan=1)

        label1 = Label(text="ผลลัพธ์", font=20, fg="white", bg="black")
        label1.grid(row=2, column=3, columnspan=1, pady=20)

        insertionv = StringVar()
        insertionv.set("")
        insertionspace1 = Entry(font=2)
        insertionspace1.grid(row=1, column=4, columnspan=3)

        insertionspace2 = Entry(font=2, textvariable=insertionv)  # เพิ่มตัวแปร textvariable
        insertionspace2.grid(row=2, column=4, columnspan=3)

        btcal = Button(windowInsertionsort, text="คำนวณ", font=20, fg="white", bg="green", command=insertionsortcal)
        btcal.grid(row=5, column=4, columnspan=1, padx=20)
...
def Selectionsort():
    ...
    # ใส่ข้อความบรรทัดแรก
    text1 = Label(text="การเรียงลำดับข้อมูลแบบเลือก (Selection Sort)", fg="green", font="25")
    text1.grid(row=0, column=0, columnspan=5, padx=90, pady=20)

    # ข้อความ "เริ่มการเรียงข้อมูล"
    text2 = Label(text="เริ่มการเรียงข้อมูล", fg="green", font=25)
    text2.grid(row=1, column=0, columnspan=1)

    # ช่องใส่ข้อมูล
    sort = Entry(width=30, font=5)
    sort.grid(row=1, column=1, columnspan=1)

    # ข้อความตัวอย่าง
    text3 = Label(text="ตัวอย่าง :  5 4 9 7 1", fg="black", bg="gray", font=5)
    ...
    # ปุ่ม "ยืนยัน"
    button1 = Button(text="ยืนยัน", font=5, command=sort_data)
    button1.place(x=90, y=120)

    # ปุ่ม "ยกเลิก"
    button2 = Button(text="ยกเลิก", font=5, command=Selection.quit)
...
def BinarySearch():
    ...
    #ใสข้อความบรรทัดแรก
    text1 = Label(text="การค้นหาข้อมูลแบบทวิภาค (Binary Search)",fg="green",font=25) 
    text1.grid(row=0,column=0,columnspan=5,padx=90,pady=20)
    ...
    #ข้อความตัวอย่าง
    text3 =Label(text="ตัวอย่าง :  5 16 99 25 36",fg="black",background="gray")
    text3.grid(row=2,column=1,pady=3)

    #ปุ่มกด
    Button1 = Button(text="ยืนยัน",fg="blue",font=15)
    Button1.place(x=200,y=135)
    Button2 = Button(text="ยกเลิก",fg="gold",font=15)
    Button2.place(x=280,y=135)
